...
Flutter compatibility currently requires special care. Flutter's `flutter doctor --android-licenses` does not recognize the license output from Command-Line Tools 23.0 and later. Flutter issue #191558 reports that Command-Line Tools 23.0 and later are not recognized by lutter doctor --android-licenses; use Command-Line Tools 22.0 until Flutter's Android tool support is updated. The legacy tool can install that version with:
