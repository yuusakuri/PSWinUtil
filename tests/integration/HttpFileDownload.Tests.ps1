...
Describe 'Android Command-Line Tools resumable download' {
    It 'installs a package after an interrupted transfer' {
        Add-Type -AssemblyName 'System.IO.Compression.FileSystem' -ErrorAction Stop
        $packageSource = Join-Path -Path $TestDrive -ChildPath 'command-line-tools-package'
        $sdkManagerDirectory = Join-Path -Path $packageSource -ChildPath 'cmdline-tools\bin'
        $packagePath = Join-Path -Path $TestDrive -ChildPath 'command-line-tools.zip'
        $sdkPath = Join-Path -Path $TestDrive -ChildPath 'AndroidSdk'
        ...
        $firstLength = [int]($packageBytes.Length / 3)
        $server = [PSWinUtil.Tests.DisconnectingHttpServer]::new(
            $packageBytes,
            [int[]]@($firstLength)
        )
        Mock -CommandName Get-WUAndroidCommandLineToolsPackage -ModuleName PSWinUtil -MockWith {
            [pscustomobject]@{
                Uri = $server.Uri
                FileName = 'command-line-tools.zip'
        ...
        try {
            ...
            $installedSdkManager = Join-Path `
                -Path $sdkPath `
                -ChildPath 'cmdline-tools\latest\bin\sdkmanager.bat'
            Test-Path -LiteralPath $installedSdkManager -PathType Leaf | Should -BeTrue
            $server.RangeStarts | Should -HaveCount 2
            $server.RangeStarts[0] | Should -Be -1
            $server.RangeStarts[1] | Should -Be $firstLength
            $server.ServerException | Should -BeNullOrEmpty
