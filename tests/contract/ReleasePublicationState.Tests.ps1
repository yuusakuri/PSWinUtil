...
Describe 'Get-ReleasePublicationState' {
    ...
    It 'reports a tag that already points at the release commit' {
        $state = Get-ReleasePublicationState -TagName 'v1.2.3' -Version '1.2.3' -ReleaseCommit $script:ReleaseCommit -TagCommit $script:ReleaseCommit
    ...
    It 'reports a completed release so a rerun skips every publication' {
        $state = Get-ReleasePublicationState -TagName 'v1.2.3' -Version '1.2.3' -ReleaseCommit $script:ReleaseCommit -TagCommit $script:ReleaseCommit -GalleryExists -GitHubReleaseExists
    ...
    It 'rejects a tag that points at another commit' {
        {
            Get-ReleasePublicationState -TagName 'v1.2.3' -Version '1.2.3' -ReleaseCommit $script:ReleaseCommit -TagCommit '2222222222222222222222222222222222222222'
        } | Should -Throw "*does not point to release commit $($script:ReleaseCommit)*"
    }

    It 'rejects a published Gallery version without its tag' {
        {
            Get-ReleasePublicationState -TagName 'v1.2.3' -Version '1.2.3' -ReleaseCommit $script:ReleaseCommit -GalleryExists
        } | Should -Throw '*exists without tag v1.2.3*'
    }

    It 'rejects a GitHub Release published before its tag' {
        {
            Get-ReleasePublicationState -TagName 'v1.2.3' -Version '1.2.3' -ReleaseCommit $script:ReleaseCommit -GitHubReleaseExists
    ...
    It 'rejects a GitHub Release published before the Gallery version' {
        {
            Get-ReleasePublicationState -TagName 'v1.2.3' -Version '1.2.3' -ReleaseCommit $script:ReleaseCommit -TagCommit $script:ReleaseCommit -GitHubReleaseExists
        } | Should -Throw '*before its tag and Gallery publication are complete*'
    }
}
