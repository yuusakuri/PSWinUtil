...
Describe 'Get-ReleaseIdentity' {
    ...
    It 'resolves the version and tag from the release branch' {
        $identity = Get-ReleaseIdentity -Branch "release/$($script:ManifestVersion)" -ManifestPath $script:ManifestPath
    ...
    It 'accepts a tag that already exists for the same release' {
        $identity = Get-ReleaseIdentity -Branch "release/$($script:ManifestVersion)" -ManifestPath $script:ManifestPath -ExistingTagName @("v$($script:ManifestVersion)", 'v1.0.0')

        $identity.Version | Should -Be $script:ManifestVersion
    }

    It 'accepts an empty tag list' {
        $identity = Get-ReleaseIdentity -Branch "release/$($script:ManifestVersion)" -ManifestPath $script:ManifestPath -ExistingTagName @()

        $identity.Version | Should -Be $script:ManifestVersion
    }

    It 'ignores tags that are not supported release tags' {
        $identity = Get-ReleaseIdentity -Branch "release/$($script:ManifestVersion)" -ManifestPath $script:ManifestPath -ExistingTagName @('v9.9.9-preview.1', 'v09.9.9', 'nightly', 'v1.0.0')
    ...
    It 'rejects a branch version that does not match the manifest version' {
        {
            Get-ReleaseIdentity -Branch 'release/99.98.97' -ManifestPath $script:ManifestPath
    ...
    It 'rejects a version older than an existing release tag' {
        {
            Get-ReleaseIdentity -Branch "release/$($script:ManifestVersion)" -ManifestPath $script:ManifestPath -ExistingTagName @('v99.0.0')
        } | Should -Throw '*is older than existing tag v99.0.0*'
    }

    It 'treats a stable version as newer than its preview' {
        $identity = Get-ReleaseIdentity -Branch 'release/2.0.0' -ManifestVersion '2.0.0' -ExistingTagName @('v2.0.0-preview9')
    ...
    It 'rejects a preview older than an existing preview' {
        {
            Get-ReleaseIdentity -Branch 'release/2.0.0-preview1' -ManifestVersion '2.0.0-preview1' -ExistingTagName @('v2.0.0-preview2')
