...
if ($Arguments) {
    ...
    $powershellArgs += "-Command `"`$Arguments = Import-Clixml -LiteralPath $ArgumentsPath; '$tempDirPath' | Where-Object { Test-Path -LiteralPath `$_ } | Remove-Item -LiteralPath { $_ } -Recurse -Force; . '$ScriptPath' @Arguments`""
}
else {
