function Set-Content {
    ...
    .EXAMPLE
    Set-Content -Path 'C:\Data\message.txt' -Value 'message'

    Writes UTF-8 without BOM and uses LF for line endings.
    ...
    dynamicparam {
        Get-WUFileCommandDynamicParameter -CommandName 'Set-Content' -BoundParameter $PSBoundParameters
    }
    ...
    begin {
        ...
        $approved = $PSCmdlet.ShouldProcess($targetDescription, 'Set file content')
        if (-not $approved) {
            return
        }
