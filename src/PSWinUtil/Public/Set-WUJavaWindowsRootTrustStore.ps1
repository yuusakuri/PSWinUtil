function Set-WUJavaWindowsRootTrustStore {
    ...
    $shouldProcessParameters = Select-WUBoundParameter `
        ...
        -Name 'WhatIf', 'Confirm'
    $trustStorePattern = '(?<!\S)-Djavax\.net\.ssl\.trustStore=\S+(?:\s+|$)'
    $trustStoreTypePattern = '(?<!\S)-Djavax\.net\.ssl\.trustStoreType=\S+'
    ...
    foreach ($currentScope in $Scope) {
        ...
        } elseif ($updatedJavaToolOptions -match $trustStoreTypePattern) {
            $updatedJavaToolOptions = [regex]::Replace(
                $updatedJavaToolOptions,
                $trustStoreTypePattern,
                $trustStoreTypeOption,
                [System.Text.RegularExpressions.RegexOptions]::IgnoreCase
            )
        } else {
